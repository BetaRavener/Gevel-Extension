...
// --- Macros for making functions visible to PostgreSQL ---
PGDLLEXPORT Datum gist_print(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(gist_print);

PGDLLEXPORT Datum gist_tree(PG_FUNCTION_ARGS);
PG_FUNCTION_INFO_V1(gist_tree);

PGDLLEXPORT Datum gist_stat(PG_FUNCTION_ARGS);
...
static void
setup_firstcall(FuncCallContext  *funcctx, Oid relOid) {
	MemoryContext     oldcontext;
	TypeStorage     *st;
	TupleDesc            tupdesc;
    ...
    st->relOid = relOid;
	st->index = index_open(relOid, AccessExclusiveLock);
	...
	st->nulls = (bool *) palloc((tupdesc->natts+2) * sizeof(bool));
...
gist_print(PG_FUNCTION_ARGS) {
	...
	if (SRF_IS_FIRSTCALL()) {
		...
		setup_firstcall(funcctx, relOid);
	}
	...
	htuple = heap_form_tuple(funcctx->attinmeta->tupdesc, st->dvalues, st->nulls);
